...
The production canonical defaults to `https://agentcomms.dev`, while documentation defaults to `https://docs.agentcomms.dev`. Override them with `NEXT_PUBLIC_SITE_URL` and `NEXT_PUBLIC_DOCS_URL`. Product version metadata comes from `PUBLIC_PRODUCT_VERSION` or the most recent accessible repository tag.
