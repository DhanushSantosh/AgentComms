...
func TestServerInfoRoundTrips(t *testing.T) {
	...
	want := ServerInfo{BaseURL: "http://127.0.0.1:4098"}
...
func TestBrokerRejectsConflictingRuntimeRegistration(t *testing.T) {
	...
	config := ProcessConfig{Executable: executable, WorkDir: t.TempDir(), Sandbox: "workspace-write"}
	if _, err := client.Register(context.Background(), "runtime-one", config); err != nil {
		t.Fatal(err)
	}
...
func TestBrokerRegisterReturnsBoundThreadID(t *testing.T) {
	...
	client := New(server.URL)
	config := ProcessConfig{Executable: executable, WorkDir: t.TempDir(), Sandbox: "workspace-write"}
	threadID, err := client.Register(context.Background(), "runtime-two", config)
	if err != nil {
		t.Fatal(err)
	}
	if threadID != "fake-thread-1" {
		t.Fatalf("Register() thread ID = %q, want fake-thread-1", threadID)
	}
}
...
func TestBrokerPromptAndLiveSubscription(t *testing.T) {
	...
	client := New(server.URL)
	config := ProcessConfig{Executable: executable, WorkDir: t.TempDir(), Sandbox: "workspace-write"}
	if _, err := client.Register(context.Background(), "runtime-live", config); err != nil {
		t.Fatal(err)
	}
