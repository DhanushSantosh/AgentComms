...
// TestManualSmokeOpenCodeACP is a throwaway, human-run verification against
// the real opencode binary, not part of the normal suite: it runs a live
// invocation through the full Worker pipeline (claim, execute, publish
// result) using whatever OpenCode provider credentials are configured
// locally. Skipped unless AGENTCOMMS_ACP_SMOKE=1, so it never runs in CI.
func TestManualSmokeOpenCodeACP(t *testing.T) {
