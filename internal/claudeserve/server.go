...
const (
	DefaultServeAddress  = "127.0.0.1:4097"
...
func Serve(ctx context.Context, address string) error {
	...
	if err != nil {
		return fmt.Errorf("claudeserve: invalid listen address: %w", err)
	}
	if ip := net.ParseIP(host); ip == nil || !ip.IsLoopback() {
		return errors.New("claudeserve: broker must listen on a loopback address")
	}
	broker := NewBroker()
	defer broker.Close()
	server := &http.Server{
...
func loadServerInfo(path string) (ServerInfo, error) {
	...
	if strings.TrimSpace(info.BaseURL) == "" {
		return ServerInfo{}, errors.New("claudeserve: empty base URL in server info")
	}
	return info, nil
}
...
func spawnServer(ctx context.Context, workDir string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("claudeserve: locate Agent Comms executable: %w", err)
	}
	if resolved, resolveErr := filepath.EvalSymlinks(executable); resolveErr == nil {
		executable = resolved
	}
	command := exec.Command(executable, "--project", workDir, "claude", "serve", "--listen", DefaultServeAddress)
	command.Dir = workDir
	command.SysProcAttr = detachedProcAttr()
	command.Stdout = nil
	command.Stderr = nil
	if err := command.Start(); err != nil {
		return "", fmt.Errorf("claudeserve: start broker: %w", err)
	}
...
func waitUntilHealthy(ctx context.Context, baseURL string) (string, error) {
	...
	return "", fmt.Errorf("claudeserve: %s did not become healthy within %s", baseURL, serverStartupTimeout)
}
