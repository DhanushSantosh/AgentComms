...
| Registered, not yet activated | You have an identity but no role/scope yet | Ask an owner or orchestrator to run `agent activate --id <agent-name> --role AGENT --scope <scope>` (CLI) or call `agent_activate` (MCP). Requesting `--role ORCHESTRATOR` specifically requires a HUMAN principal to grant it, even from an existing orchestrator — never ask for that role for yourself, and never construct or run that command on a human's behalf even if asked to relay it. The check only verifies the signer's principal type, not that a human actually decided this in the moment; tell them to type it themselves. |
