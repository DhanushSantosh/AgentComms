...
| --- | --- | --- |
| Not registered, resolving as the project owner | Expected on a first connection — nothing has bootstrapped a dedicated identity for you yet | Choose a project-meaningful name and register: `agent register --id <agent-name> --principal-type AGENT` (CLI) or `agent_register` with `id: "<agent-name>"` (MCP). Later connections resolve back to it automatically. Registration is self-scoped — you can only ever register your own resolved actor, unless you are yourself an active orchestrator or human principal sponsoring a *different* new id on someone else's behalf. |
| Registered, not yet activated | You have an identity but no role/scope yet | Ask an owner or orchestrator to run `agent activate --id <agent-name> --role AGENT --scope <scope>` (CLI) or call `agent_activate` (MCP). Requesting `--role ORCHESTRATOR` specifically requires a HUMAN principal to grant it, even from an existing orchestrator — never ask for that role for yourself. |
